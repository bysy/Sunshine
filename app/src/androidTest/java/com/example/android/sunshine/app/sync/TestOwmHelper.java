...
public class TestOwmHelper extends AndroidTestCase {
    ...
    @TargetApi(11)
    public void testGetOrAddLocation() {
        ...
        long locationId = OwmHelper.getOrAddLocation(context,
                ADD_LOCATION_SETTING, ADD_LOCATION_CITY,
                ADD_LOCATION_LAT, ADD_LOCATION_LON);

        // does getOrAddLocation return a valid record ID?
        assertFalse("Error: getOrAddLocation returned an invalid ID on insert",
                locationId == -1);
        ...
        for ( int i = 0; i < 2; i++ ) {
            ...
            if (locationCursor.moveToFirst()) {
                assertEquals("Error: the queried value of locationId does not match the returned value" +
                        "from getOrAddLocation", locationCursor.getLong(0), locationId);
            ...
            long newLocationId = OwmHelper.getOrAddLocation(context,
                    ADD_LOCATION_SETTING, ADD_LOCATION_CITY,
                    ADD_LOCATION_LAT, ADD_LOCATION_LON);
